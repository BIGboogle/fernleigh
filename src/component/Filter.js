...
export default function Filter({ styles, category }) {
    const style = styles.filter((s) => s.category === category)

    return (
        <div>
            <div className="flex items-center justify-between mb-2">
                <h1 className="text-xl font-alt capitalize">{category}</h1>
                <Link href={`/catalog/category/${category}`} className="flex items-center font-text gap-1">See all
                    <ArrowRight size={15} />
                </Link>
            </div>
                        ...
                        <Link key={item.id} href={`/catalog/${item.id}`} className="relative min-w-[200px] h-[280px] rounded-xl overflow-hidden">
                            ...
                            <div className="flex items-center pl-2 p-2 py-3 text-sm absolute inset-0 top-auto bg-black/10 backdrop-blur-md">
                                <div className="flex gap-2 flex-col">
                                    <p className=" text-white font-text w-full  truncate">{item.name}</p>
